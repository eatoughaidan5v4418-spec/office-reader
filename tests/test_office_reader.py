  ...
  <Relationship Id="rIdBoard" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/board.png"/>
    ...
    def test_unified_reader_writes_review_items_artifact(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("review_items_markdown", stdout)
            review_path = Path(stdout["review_items"])
            review_csv_path = Path(stdout["review_items_csv"])
            review_md_path = Path(stdout["review_items_markdown"])
            ...
            checklist = review_md_path.read_text(encoding="utf-8")
            self.assertIn("# Review Checklist", checklist)
            self.assertIn("- [ ] Comment comment-0", checklist)
            self.assertIn("Please verify the ARR source.", checklist)
            self.assertIn("location: paragraph 2", checklist)
            self.assertIn("- [ ] Revision revision-3", checklist)
            self.assertIn("location: table 1 row 2 cell 2", checklist)
            ...
            self.assertEqual(manifest["artifacts"]["review_items_markdown"], str(review_md_path))
            report = (out_dir / "board-memo.report.md").read_text(encoding="utf-8")
